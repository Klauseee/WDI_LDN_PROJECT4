module.exports = [{
  title: 'CEO',
  location: 'London',
  type: 'permanent',
  technologies: {
    primary: ['javascript', 'html', 'css'],
    secondary: ['nodejs', 'rails', 'angularjs', 'mongodb']
  },
  summary: 'Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum.',
  salary: 5000000
}, {
  title: 'Web Developer',
  location: 'Manchester',
  type: 'permanent',
  technologies: {
    primary: ['html', 'css', 'javascript'],
    secondary: ['angularjs', 'krakenjs', 'ruby', 'php']
  },
  summary: 'Hamburger ball tip short ribs meatball sirloin shank biltong jerky pork loin beef ribs burgdoggen leberkas. Andouille ham hock flank sirloin, bresaola pancetta doner. Tail pork loin beef tenderloin shank. Bresaola porchetta strip steak chicken tenderloin capicola t-bone pork loin sausage hamburger brisket tail swine. Sirloin ground round salami sausage pork meatball capicola pastrami rump boudin shoulder.',
  salary: 50000
}, {
  title: 'Rails Dev',
  location: 'London',
  type: 'contract',
  technologies: {
    primary: ['rails', 'javascript', 'ruby'],
    secondary: ['react', 'mongodb', 'erlang', 'wordpress']
  },
  summary: 'Bacon ipsum dolor amet burgdoggen tail flank, shoulder tongue cupim shankle pork chop tenderloin pork loin pig. Kielbasa bresaola picanha sausage, jowl salami frankfurter fatback. Frankfurter meatball hamburger pork chop. Prosciutto pork pork belly shoulder.',
  salary: 67000
}, {
  title: 'React Dev',
  location: 'Liverpool',
  type: 'permanent',
  technologies: {
    primary: ['react', 'javascript', 'css'],
    secondary: ['krakenjs', 'mongodb', 'erlang', 'wordpress']
  },
  summary: 'Ball tip tri-tip tenderloin flank chuck spare ribs shank pig tail strip steak kevin capicola filet mignon. Pancetta drumstick bacon, swine bresaola ball tip ground round pork loin beef ribs shankle jowl burgdoggen short loin jerky turducken. Porchetta brisket tenderloin meatloaf meatball. Jowl chuck landjaeger doner.',
  salary: 75000
}, {
  title: 'Data Architect',
  location: 'London',
  type: 'permanent',
  technologies: {
    primary: ['html', 'javascript', 'css'],
    secondary: ['react', 'mongodb', 'jquery', 'laravel']
  },
  summary: 'Cow tail ham hock doner meatloaf spare ribs. Ribeye flank burgdoggen short loin beef cupim, salami sirloin swine meatloaf. Landjaeger jerky pork chop tongue bresaola kevin sausage fatback porchetta pork drumstick burgdoggen cupim prosciutto boudin. Cow landjaeger pork burgdoggen. Chicken alcatra cupim, beef flank shank venison.',
  salary: 180000
}, {
  title: 'Junior Web Developer',
  location: 'London',
  type: 'contract',
  technologies: {
    primary: ['html', 'javascript', 'css'],
    secondary: ['react', 'mongodb', 'jquery']
  },
  summary: 'Boudin ham hock shoulder filet mignon. Cupim chicken buffalo capicola kevin short ribs spare ribs tri-tip venison shoulder chuck turducken landjaeger. Frankfurter andouille ground round prosciutto tenderloin shank meatball landjaeger pastrami bresaola jerky. Buffalo landjaeger frankfurter fatback, brisket beef cupim ham hock bresaola short loin andouille cow tail strip steak jowl. Shank beef prosciutto landjaeger hamburger short ribs capicola porchetta tenderloin buffalo spare ribs jerky.',
  salary: 28000
}, {
  title: 'Full Stack Developer',
  location: 'London',
  type: 'contract',
  technologies: {
    primary: ['html', 'javascript', 'css'],
    secondary: ['react', 'mongodb', 'jquery']
  },
  summary: 'Rump swine alcatra pork belly meatball ham filet mignon ball tip pastrami salami landjaeger t-bone. Pork loin sirloin burgdoggen porchetta flank. Ball tip biltong sausage shankle swine chicken prosciutto tri-tip hamburger. Biltong shankle ball tip meatloaf prosciutto, shank pork belly. Landjaeger shank cupim, meatball tri-tip sausage beef ribs chicken ribeye frankfurter biltong shankle. Pork chop short ribs biltong, pork loin turkey sirloin shank ribeye.',
  salary: 54000
}];
