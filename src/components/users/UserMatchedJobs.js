      ...
      {jobs && jobs.map(job =>
        <div key={job.title} className="card">
          <div className="card-content">
            ...
            <button className="button is-primary" onClick={() => handleApply(job._id)}>Accept</button>
            <button className="button is-danger" onClick={() => handleDismiss(job._id)}>Dismiss</button>
          </div>
        </div>
      )}
    </div>
